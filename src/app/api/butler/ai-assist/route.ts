...
const CAPTION_PROMPT = `你是長照機構的管家助手。請根據這張服務照片，寫出一句 10-20 字的客觀說明文字，描述照片中的服務內容或情境。
要求：
- 只描述照片中實際看到的內容，不要臆測
- 直接輸出說明文字，不要加任何前綴或說明`

const SUMMARY_PROMPT = `以下是同一位住戶在一段期間內的多篇服務紀錄草稿，請將它們濃縮改寫成一篇連貫的整體紀錄書面文字，呈現這段期間的狀況變化與重點。
規則：
- 只能輸出濃縮後的紀錄本文，禁止輸出任何講解、建議、版本選項、標題、前言或結語
- 不捏造原始紀錄中沒有的資訊，不新增未提及的觀察項目
- 依時間脈絡整合重複或相似的描述，避免逐日流水帳式羅列
- 輸出必須是純文字，不可使用 Markdown 符號（*, #, > 等）`
...
export async function POST(req: NextRequest) {
  ...
  const { mode, text, imageUrl, tags, entries } = await req.json()
  ...
  try {
    ...
    if (mode === 'summary') {
      if (!Array.isArray(entries) || entries.length === 0) {
        return NextResponse.json({ error: '缺少可彙整的紀錄內容' }, { status: 400 })
      }
      const body = entries
        .map((e: { date?: string; text: string }) => `【${e.date ?? ''}】\n${e.text}`)
        .join('\n\n')
      const result = await callGemini([{ text: `${SUMMARY_PROMPT}\n\n原始紀錄：\n${body}` }])
      return NextResponse.json({ result })
    }
