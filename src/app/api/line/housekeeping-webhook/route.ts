...
import { generateHousekeepingReport, generateEODReport, generateUnlockReport, generateUnlock2Report } from '@/lib/line/housekeeping-report'
...
const isEOD      = EOD_KEYWORDS.some(k => text.includes(k))
    ...
    const isUnlock2  = UNLOCK2_KEYWORDS.some(k => text.includes(k))
    const isKeyword  = KEYWORDS.some(k => text.includes(k))
    if (!isEOD && !isUnlock && !isUnlock2 && !isKeyword) continue
