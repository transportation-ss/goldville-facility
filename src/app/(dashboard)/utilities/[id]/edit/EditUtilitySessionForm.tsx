...
import { updateUtilitySession } from './actions'
import type { MeterWithHistory } from '../../new/page'

const ANOMALY_THRESHOLD = 1.2

const SECTIONS = [
  { key: 'water-all', label: '頂樓 — 全館水表', icon: 'water' as const, filter: (m: MeterWithHistory) => m.meter_type === 'water' },
  { key: 'elec-2f', label: '2F 電表', icon: 'elec' as const, filter: (m: MeterWithHistory) => m.meter_type === 'electricity' && m.floor === '2F' },
  { key: 'elec-3f', label: '3F 電表', icon: 'elec' as const, filter: (m: MeterWithHistory) => m.meter_type === 'electricity' && m.floor === '3F' },
  { key: 'elec-5f', label: '5F 電表', icon: 'elec' as const, filter: (m: MeterWithHistory) => m.meter_type === 'electricity' && m.floor === '5F' },
  { key: 'elec-6f', label: '6F 電表', icon: 'elec' as const, filter: (m: MeterWithHistory) => m.meter_type === 'electricity' && m.floor === '6F' },
  { key: 'elec-7f', label: '7F 電表', icon: 'elec' as const, filter: (m: MeterWithHistory) => m.meter_type === 'electricity' && m.floor === '7F' },
  {
    key: 'elec-common', label: '公共電表', icon: 'elec' as const,
    filter: (m: MeterWithHistory) => m.meter_type === 'electricity' && !['2F', '3F', '5F', '6F', '7F'].includes(m.floor ?? ''),
  },
]
...
export function EditUtilitySessionForm({
  sessionId, meters, initialValues, initialDate, initialNotes, initialStatus,
}: Props) {
  ...
  const handleSave = async (status: 'draft' | 'complete') => {
    ...
    try {
      ...
      await updateUtilitySession(sessionId, { readingDate, specialNotes, status, readings })
    } catch (e) {
  ...
  return (
    <div className="pb-48 md:pb-32">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900">編輯抄表</h1>
          <p className="text-xs text-gray-500 mt-0.5">
            原狀態：{initialStatus === 'complete' ? '✅ 完整' : '📝 草稿'}
          </p>
        </div>
