...
import { getResidents, getAllActiveResidentServices } from '../residents/actions'
...
export default async function ButlerPlanPage({
  ...
  const [tasks, staff, residents, residentServices] = await Promise.all([
    getButlerTasksByDate(viewDate),
    getButlerStaff(),
    getResidents(),
    getAllActiveResidentServices(),
  ])
  const activeResidents = residents
    .filter(r => r.status === 'active_resident')
    .map(r => ({ id: r.id, name: r.name, room: r.room }))

  return <ButlerPlanView today={today} viewDate={viewDate} tasks={tasks} staff={staff}
    residents={activeResidents} residentServices={residentServices} />
}
