...
// ── 任務 Modal ────────────────────────────────────────────
function SlotModal({
  staffId, startTime, today, staff, existingTask, onClose,
}: {
  staffId: string
  startTime: string
  today: string
  staff: ButlerStaff[]
  existingTask?: ButlerTask | null
  onClose: () => void
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (existingTask) {
        await updateButlerTask(existingTask.id, payload)
      } else {
  ...
  async function handleDelete() {
    ...
    setSaving(true)
    try {
      await deleteButlerTask(existingTask.id)
      onClose()
    } finally {
      setSaving(false)
    }
    ...
    <div className="px-4 py-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs text-gray-400 border rounded-lg px-2 py-1"
        >
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[600px]">
          ...
          {/* 人員列 */}
          {staff.map(s => {
            const staffTasks = tasks.filter(t => t.assigned_to === s.id && t.start_time)

            return (
              <div key={s.id} className="flex items-center mb-1 h-10">
                <div className="w-20 shrink-0 text-xs text-gray-600 font-medium pr-2 truncate">
                  {s.display_name}
                </div>
                {/* 格子 */}
                <div className="flex-1 relative flex h-full">
                  {slots.map(slot => {
                    // 找這個 slot 是否被某個任務佔用
                    const slotTask = staffTasks.find(t => {
                      const ts = timeToSlot(t.start_time!)
                      const te = ts + Math.round((t.duration_minutes ?? 30) / 30)
                      return slot >= ts && slot < te
                    })
                    const isTaskStart = slotTask && timeToSlot(slotTask.start_time!) === slot

                    if (slotTask && !isTaskStart) return null // 被前一格的任務 span

                    const spanSlots = slotTask
                      ? Math.round((slotTask.duration_minutes ?? 30) / 30)
                      : 1
                    ...
                    return (
                      <div
                        ...
                        style={{ width: `${spanSlots * (100 / slots.length)}%` }}
                        onClick={() => setModal({
                          staffId: s.id,
                          startTime: slotToLabel(slot),
                          task: slotTask ?? null,
                        })}
                      >
                        {slotTask && isTaskStart && (
          ...
          <div className="bg-white border rounded-xl divide-y">
            {tasks.filter(t => !t.start_time).map(t => (
              <div key={t.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <button
                  onClick={() => setModal({ staffId: t.assigned_to ?? '', startTime: '09:00', task: t })}
